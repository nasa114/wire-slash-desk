...
| `UI_PASSWORD` | | — | ブラウザ用 read-only UI(`/ui`)の Basic 認証パスワード。**未設定なら UI は無効(404)**。生成例: `openssl rand -hex 16` |
...
### ブラウザからの確認(read-only UI)

MCP クライアントが使えない緊急時でも状況確認できるよう、最小の読み取り専用 UI がある(書き込み操作なし。本格的な管理 UI は T4-1 / 未決事項 U3 の確定後)。

1. `.env` に `UI_PASSWORD=<openssl rand -hex 16 で生成>` を設定してサーバーを起動(未設定なら `/ui` は 404)
2. ブラウザで `http://localhost:3000/ui` を開く(Dev Container なら VS Code のポート転送経由)
3. Basic 認証プロンプトにユーザー名は任意、パスワードに `UI_PASSWORD` の値を入力

- `/ui` … 記事一覧(タイトル検索・日付絞り込み・フィード絞り込み)
- `/ui/feeds` … フィード一覧(有効/無効・本文取得可否・最終取得時刻)
- 認証系統は MCP(`MCP_BEARER_TOKEN`)・収集(`COLLECTOR_TOKEN`)と完全に分離されており、UI パスワードで API は呼べない
...
- **U3**: 管理UI — 最小SSR / Hono+HTMX / Next.js 等の技術選定。
