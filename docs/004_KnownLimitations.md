...
## 7. `/login` にレート制限・アカウントロックなし

- **現状**: オンライン総当たりに対する防御は scrypt の計算コスト（1試行
  ~100ms）と、ユーザー名列挙を防ぐダミーハッシュ検証のみ
  （`src/server/web.ts`, `src/server/password.ts`）。試行回数制限・
  ロックアウト・監査ログはない。
- **リスク**: 公開網に置くと辞書攻撃・パスワードスプレーが時間をかければ可能。
- **対応案**: IP/ユーザー名単位のメモリ内スロットリング（数行で足りる）、
  またはリバースプロキシ（nginx `limit_req` / Cloudflare 等）での制限。
  強いパスワード（パスフレーズ）の使用を前提とする。
- **トリガー条件**: インターネットから到達可能な環境への配置時。
