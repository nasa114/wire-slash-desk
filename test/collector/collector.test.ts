...
test('Atomフィード: <id> がそのまま guid として採用される(item.guid ではなく item.id に入るため)', async () => {
  const repos = createMemoryRepositories();
  await seedFeed(repos, { feedUrl: 'https://example.com/atom.xml' });
  const fetchFn = createFakeFetch(() => xmlResponse(ATOM_FIXTURE));
  ...
  const first = articles.find((a) => a.url === 'https://example.com/atom/1');
  assert.ok(first, 'id つきの Atom エントリが保存される');
  assert.equal(
    first?.guid,
    'atom-entry-1',
    'rss-parser は Atom の <id> を item.id に格納するため、guid には item.id が採用されるべき',
  );

  const second = articles.find((a) => a.url === 'https://example.com/atom/2');
  assert.ok(second, 'guid/id 両方欠落のエントリも url があれば保存される');
  assert.equal(
    second?.guid,
    sha256Hex('https://example.com/atom/2'),
    'guid/id 両方欠落時は sha256(url) にフォールバックする',
  );
});

test('タイムアウト: timeoutMs 超過で fetch に渡された signal が abort し、対象フィードは status=error になるが他フィードは収集される', async () => {
  const repos = createMemoryRepositories();
  const slowFeed = await seedFeed(repos, { feedUrl: 'https://slow.example.com/rss.xml' });
  const goodFeed = await seedFeed(repos, { feedUrl: 'https://fast.example.com/rss.xml' });

  const fetchFn = createFakeFetch((url, _headers, _callIndex, signal) => {
    if (url === slowFeed.feedUrl) {
      // 決して自力では解決/拒否しない Promise。signal の abort イベントでのみ reject する。
  ...
  const slowResult = result.feeds.find((f) => f.feedId === slowFeed.id);
  const goodResult = result.feeds.find((f) => f.feedId === goodFeed.id);

  assert.equal(slowResult?.status, 'error', 'タイムアウトしたフィードは error になる');

  const slowCall = fetchFn.calls.find((c) => c.url === slowFeed.feedUrl);
  assert.ok(slowCall?.signal, 'fetchFn には AbortSignal が渡されている必要がある');

  assert.equal(goodResult?.status, 'ok', '他フィードの収集は妨げられない');
  assert.equal(goodResult?.inserted, 2);
});
