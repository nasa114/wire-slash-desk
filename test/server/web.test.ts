...
async function startApp(opts: { setupToken?: string } = {}): Promise<TestApp> {
...
test('PT-001: 並行 setup POST でも管理ユーザーは1件しか作成されない(原子的 first-run)', async () => {
  ...
  try {
    ...
    const created = results.filter((r) => r.status === 303);
    const rejected = results.filter((r) => r.status === 403);
    assert.equal(created.length, 1, '成功は1件のみ');
    assert.equal(rejected.length, 7, '残りは 403(setup already completed)');
...
test('PT-001: SETUP_TOKEN 設定時はトークン一致がないと管理ユーザーを作成できない', async () => {
  const { base, repos, close } = await startApp({ setupToken: 'secret-setup-token' });
  try {
    // GET はトークン入力欄を表示する。
    const page = await getPage(base, '/setup');
    assert.equal(page.status, 200);
    assert.match(await page.text(), /name="setup_token"/);

    // トークンなし → 403、作成されない。
    const noToken = await postForm(base, '/setup', {
      username: 'admin',
      password: 'long-enough-password',
      password_confirm: 'long-enough-password',
    });
    assert.equal(noToken.status, 403);
    assert.equal(await repos.users.count(), 0, 'トークンなしでは作成されない');

    // 誤トークン → 403。
    const wrong = await postForm(base, '/setup', {
    ...
    // 正トークン → 作成成功。
    const ok = await postForm(base, '/setup', {
...
test('login: 背景・アラートJSはログイン画面のみ(setup・認証後ページには出ない)', async () => {
  ...
  try {
    ...
    // 内容ハッシュ版キャッシュバスティング(asset-cache.test.ts)により ?v= が付く。
    assert.match(loginHtml, /src="\/assets\/login\.js(\?v=[0-9a-f]{12})?"/);
...
test('assets: app.css が配信され、ページはインラインCSSを持たない', async () => {
  ...
  try {
    ...
    // 内容ハッシュ版キャッシュバスティング(asset-cache.test.ts)により ?v= が付く。
    assert.match(loginHtml, /<link rel="stylesheet" href="\/assets\/app\.css(\?v=[0-9a-f]{12})?">/);
